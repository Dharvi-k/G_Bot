...
class GitaChatbot:
    def __init__(self, data_path='gita_translation_data.json'):
        ...
        self.embeddings = self.embedding_model.encode(self.df['cleaned_verse'], convert_to_numpy=True).astype('float32')
    ...
    def _prepare_intent_data(self):
        ...
        for label, samples in self.intents.items():
            for s in samples:
                vec = self.embedding_model.encode(s, convert_to_numpy=True)
                X.append(vec)
                y.append(label)
        return np.array(X), np.array(y)

    def detect_intent(self, query):
        vec = self.embedding_model.encode(query, convert_to_numpy=True).reshape(1, -1)
    ...
    def retrieve_verses(self, query, k=3):
        ...
        query_emb = self.embedding_model.encode(cleaned_query, convert_to_numpy=True).astype('float32').reshape(1, -1)
    ...
    def generate_explanation(self, query, retrieved_verses):
        ...
        return (retrieved_verses, response[0]["generated_text"]) if retrieved_verses else (None, response[0]["generated_text"])
